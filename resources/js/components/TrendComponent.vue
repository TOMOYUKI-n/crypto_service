...
<div class="l-main-container">
    ...
    <div class="c-main__function-head">
        <div class="p-trend__time">
            <p class="no-padding">情報取得日時：{{ trends[0]['get_dates'] }}</p>
        </div>
        ...
        <div class="p-trend__search">
            <div class="p-trend__search-time">
                ...
                <form class="">
                    <select class="p-trend__search-time__select"
                        aria-label="Search" 
                        v-model="keyword" name="keyword" value="keyword">
                        <option value="" selected>選択してください</option>
                        <option value="1hour">過去1時間でのランキング</option>
                        <option value="1day">過去1日間でのランキング</option>
                        <option value="1week">過去1週間でのランキング</option>
                    </select>
                    <button type="submit" v-on:click="getSearch()">
                        Search
                    </button>
                </form>
            </div>
    ...
    <div class="p-trend__list-wrap">
        ...
        <section v-if="errored">
          <p>情報が取得できませんでした。再度時間を置いて更新してください。</p>
        </section>

        <section v-else>
            <div v-for="trend in trendData" 
                v-bind:key="trend.index" 
                class="p-trend__list-title" >
...
export default {
    ...
    data: function() {
        return {
            ...
            selected: [],
            options: [ 
                    'BTC','ETH','ETC','LSK','FCT','XRP',
                    'XEM','LTC','BCH','MONA','XLM',
                    'QTUM','DASH','ZEC','XMR','REP',
            ]
        }
    },
    methods: {
        ...
        filtered(){
            ...
            for (var i = 0 ; i < this.selected.length; i++){
		        result[i] = this.trends.filter(word => word['coin_name'] == this.selected[i]);
            };
            // ネストされた配列のフラット化
